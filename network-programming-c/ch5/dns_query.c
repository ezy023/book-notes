...
const unsigned char *print_name(const unsigned char *msg,    // pointer to message's beginning
                                const unsigned char *p,      // pointer to the name to print
                                const unsigned char *end) {  // point to one past the end of message
    if (p + 2 > end) {
    ...
    if ((*p & 0xC0) == 0xC0) {
        const int k = ((*p & 0x3F) << 8) + p[1]; // get the six bits in the remainder of the byte, and the entire next byte, as the pointer value
        p += 2;
        printf(" (pointer %d) ", k);
        print_name(msg, msg+k, end);
        return p;
    } else {
        const int len = *p++;       // store length of current label
        if (p + len + 1 > end) {
            fprintf(stderr, "End of message.\n");
            exit(1);
        }
        printf("%.*s", len, p);
        p += len;
        if (*p) {                   // if next byte isn't 0 the name continues
            printf(".");
...
void print_dns_message(const char *message, int msg_length) {
    ...
    /* print entire raw DNS message, will output many lines
    int i;
    ...
    */
    ...
    /* Print message ID, first two bytes of the message */
    printf("ID = %0X %0X\n", msg[0], msg[1]);

    /* Get QR bit from message header */
    const int qr = (msg[2] & 0x80) >> 7;
    printf("QR 1 %d %s\n", qr, qr ? "response" : "query");

    /* Print OPCODE, AA, TC, and RD fields */
    ...
    /* Read in RCODE for response-type */
    if (qr) {                  // if this is a response message
    ...
    /* QDCOUNT (question count), ANCOUNT (answer count), NSCOUNT (name server count), ARCOUNT (additional count) */
    ...
    /* Printing Answer, authority, and additional sections */
...
int main(int argc, char *argv[]) {
    ...
    /* construct data for DNS message */
