...
int main() {
    ...
    char listen_port[10];
    if (getnameinfo(bind_address->ai_addr, bind_address->ai_addrlen, listen_addr, sizeof(listen_addr), listen_port, sizeof(listen_port), NI_NUMERICHOST | NI_NUMERICSERV)) {
        fprintf(stderr, "getnameinfo(listen_addr) failed. (%d)\n", errno);
        return 1;
    }
