...
apply-dispatch
  (test (op prmitive-procedure?) (reg proc))
  (branch (label primitive-apply))
  (test (op compound-procedure?) (reg proc))
  (branch (label compound-apply))
  ;; Update apply-dispatch to handle compiled procedures
...
compiled-apply
  (restore continue) ;; compiled code expects the continuation to be in 'continue' which is why it is restored here
...
ev-sequence-last-exp
  ...
  (goto (label eval-dispatch))
ev-if
  (save exp) ;; save if expression for later use in extracting the consequent or alternative
  (save env)
  (save continue)
  (assign continue (label ev-if-decide))
  (assign exp (op if-predicate) (reg exp)) ;; move the if predicate into 'exp' for evaluation
...
ev-if-alternative
  (assign exp (op if-alternative) (reg exp)) ;; move alternative for if expression into 'exp' for evaluation
  (goto (label eval-dispatch))
ev-if-consequent
...
ev-assignment-1
  ...
  (restore unev)
  (perform
   (op set-variable-value!) (reg unev) (reg val) (reg env)) ;; set-variable-value! is assumed to be an available machine instruction
  (assign val (const ok))
  (goto (reg continue))
ev-definition
...
;;    '(
;;      read-eval-print-loop
;;      <all machine code above>
;;      )))

;; (define eceval-operations
;;   (list (list 'self-evaluating? self-evaluating?)
;;         <all the other machine operations assumed to be available>))
...
;;   (set-the-register-contents! eceval 'flag false)
;;   (start eceval))
