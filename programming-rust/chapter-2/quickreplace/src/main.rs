...
#[derive(Debug)] // this attribute tells the compiler to generate some extra code that allows us to format the Arguments struct with `{:?}` in println!
struct Arguments {
    target: String,
    replacement: String,
    filename: String,
...
fn print_usage() {
    ...
    eprintln!("Usage: quickreplaces <target> <replacement> <INPUT> <OUTPUT>");
}
